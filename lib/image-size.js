...
function avifSize(buffer) {
  ...
  const brands = buffer.toString('ascii', 8, Math.min(buffer.length, 8 + buffer.readUInt32BE(0)));
  if (!/avif|avis|mif1|heic/.test(brands)) {
    return null;
  }

  // ponytail: scan the header for the ispe box instead of walking the ISOBMFF tree.
  // The meta box precedes the image data, so bound the scan; walk the tree if that stops holding.
  const ispe = buffer.subarray(0, Math.min(buffer.length, 4096)).indexOf('ispe');
  if (ispe === -1 || ispe + 16 > buffer.length) {
    throw new Error('Could not find AVIF dimensions.');
  }

  return assertDimensions(buffer.readUInt32BE(ispe + 8), buffer.readUInt32BE(ispe + 12), 'avif');
}
...
function imageSize(buffer) {
  ...
  const dimensions = pngSize(buffer) || gifSize(buffer) || jpegSize(buffer) || webpSize(buffer) || avifSize(buffer);
  if (!dimensions) {
    throw new Error('Unsupported image format. Expected PNG, JPEG, GIF, WebP, or AVIF.');
  }
